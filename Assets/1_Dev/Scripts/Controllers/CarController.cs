...
public class CarController : MonoBehaviour
{
    ...
    IEnumerator SimulateEarthquakeSequence()
    {
        ...
        if (target == null)
            StartCoroutine(SimulateCarShake(6f, 2f));
    ...
    IEnumerator SimulateCarShake(float shakeDuration, float shakeIntensity)
    {
        Vector3 originalPosition = transform.position;
        Quaternion originalRotation = transform.rotation;
        float elapsedTime = 0f;

        float dynamicShakeIntensity = shakeIntensity * (1 / 10f);

        while (elapsedTime < shakeDuration)
        {
            Vector3 randomRotation = new Vector3(
                Random.Range(-dynamicShakeIntensity, dynamicShakeIntensity),
                0,
                Random.Range(-dynamicShakeIntensity, dynamicShakeIntensity)
            );

            Vector3 randomOffset = new Vector3(
                Random.Range(-dynamicShakeIntensity, dynamicShakeIntensity) * 0.1f,
                0,
                Random.Range(-dynamicShakeIntensity, dynamicShakeIntensity) * 0.1f
            );

            transform.rotation = Quaternion.Euler(randomRotation) * originalRotation;
